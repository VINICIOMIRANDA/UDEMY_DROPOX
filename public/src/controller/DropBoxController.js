class DroBoxController {

    constructor() {

        this.currentFolder = ['hcode'];

        this.onselectionchange = new Event('selectionchange');//AuLA 101

        this.navEl = document.querySelector('#browse-location'); //AULA 106
        this.btnSendFileEl = document.querySelector('#btn-send-file');
        this.inputFilesEl = document.querySelector('#files');
        this.snackModalEl = document.querySelector('#react-snackbar-root');//Chamando o evento do react-snackbar-root
        this.progressBarEl = this.snackModalEl.querySelector('.mc-progress-bar-fg');
        this.namefileEl = this.snackModalEl.querySelector('.filename');
        this.timeleftEl = this.snackModalEl.querySelector('.timeleft');
        this.listFilesEl = document.querySelector('#list-of-files-and-directories'); // Carregament o elemento do ID list-of-files-and-directories do View/index.ejs AULA 98

        // Declarando os botões nova pasta , renomear e excluir
    ...
    connectFirebase() {
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyAM9gwdT1zftuor0exFLb5xuUmF2S6dRSc",
            authDomain: "dropbox-clone-a7eef.firebaseapp.com",
            databaseURL: "https://dropbox-clone-a7eef.firebaseio.com",
            projectId: "dropbox-clone-a7eef",
            storageBucket: "dropbox-clone-a7eef.appspot.com",
            messagingSenderId: "87871264869",
            appId: "1:87871264869:web:8bdb8331a7ffc707cece8c",
            measurementId: "G-TVPT0WKW5X"
        };
        // Initialize Firebase
    ...
    initEvents() {
        ...
        this.btnRename.addEventListener('click', e => {  //AULA 103 
            ...
            if (name) {
                ...
                this.getFirebaseRef().child(li.dataset.key).set(file); //AULA 103 
            }

        });


        this.listFilesEl.addEventListener('selectionchange', e => { //AULA 101

            console.log('selectionchange', this.getSelection().length); //AULA 101

            switch (this.getSelection().length) { //AULA 102 D14 - Menu de opções de acordo com arquivos selecionados
                ...
                case 1:
                    this.btnDelete.style.display = 'block'; //block aparecer
                ...
                default:
                    this.btnDelete.style.display = 'block'; //block aparecer
                    this.btnRename.style.display = 'none'; // Desaparece 
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            this.btnSendFileEl.disabled = true //travando o botão durante o upload AULA 97
            ...
            //  this.snackModalEl.style.display = 'block';   // Mostrar o evento na tela

        });

    }

    uploadComplete() { //AULA 97

        this.modalShow(false);
        this.inputFilesEl.value = '';
        this.btnSendFileEl.disabled = false; // Habilita o botão

    }

    getFirebaseRef(path) { //AULA 97 e AULA 106


        if (!path) path = this.currentFolder.join('/'); // .join() é um metodo que junto os arrays

        console.log('TESTEPATH' + path);

        // return firebase.database().ref('files');

        return firebase.database().ref(path); //AULA 106 Navegando nas pastas ou abrindo arquivos
    ...
    modalShow(show = true) {
        ...
        this.snackModalEl.style.display = (show) ? 'block' : 'none'; //Mostrar o evento na tela progresso
    ...
    ajax(url, method = 'GET', formData = new FormData(), onprogress = function () { }, onloadstart = function () { }) {
        ...
        return new Promise((resolve, reject) => {
            ...
            onloadstart();   // this.startUploadTime = Date.now();
    ...
    getFileIconView(file) { //Adicionar o FILE para identificar o metodo para o getFileView
    ...
    getFileView(file, key) { //AULA 98


        let li = document.createElement('li');

        li.dataset.key = key;
        li.dataset.file = JSON.stringify(file); //AULA 103 Renomeando arquivo
        ...
        this.initEventsLi(li); // Adicionar o evento do li  /AULA99 
    ...
    readFiles() { ////// AULA 98 
        ...
        this.lastFolder = this.currentFolder.join('/'); //AULA 106

        console.log(this.lastFolder);

        this.getFirebaseRef().on('value', snapshot => {

            this.listFilesEl.innerHTML = ''; //Limpando os elementos html

            snapshot.forEach(snapshotItem => { //Cada item ele dará acesso as informações através de um HASH

                let key = snapshotItem.key;//Propriedade
                let data = snapshotItem.val(); // Metodo

                if (data.type) {

                    this.listFilesEl.appendChild(this.getFileView(data, key)) //Adicionando os elementos
                }
        ...
        for (let i = 0; i < this.currentFolder.length; i++) {
            ...
            if ((i + 1) === this.currentFolder.length) {
                ...
                <span class="ue-effect-container uee-BreadCrumbSegment-link-0">
                    <a href="#" data-path = "${path.join('/')}" class="breadcrumb-segment">${folderName}</a></span>
    ...
    initEventsLi(li) {  //AULA 99 Selecionado Arquivos
        ...
        li.addEventListener('click', e => {
            ...
            if (e.shiftKey) {
                ...
                if (firstLi) {
                    ...
                    this.listFilesEl.dispatchEvent(this.onselectionchange); //AULA 101 Evento de mudança de seleção
                    return true; //AULA 100
            ...
            if (!e.ctrlKey) { // Se o control não estiver habilitado, remover as class selected 
